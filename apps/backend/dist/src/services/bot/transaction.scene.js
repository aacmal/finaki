...
const createTransaction = async (ctx, data) => {
    try {
        ...
        await ctx.reply("Transaksi berhasil dibuat", Markup.inlineKeyboard([
            [
...
(ctx) => {
    ...
    ctx.reply("Pilih Jenis Transaksi", Markup.keyboard([["⬆️ Masuk", "⬇️ Keluar"]])
        .oneTime()
        .resize());
    return ctx.wizard.next();
}, 
...
(ctx) => {
    ...
    if (response !== "⬆️ Masuk" && response !== "⬇️ Keluar") {
        ctx.reply("Pilih salah satu jenis transaksi", Markup.keyboard([
...
async (ctx) => {
    ...
    if (response === "Ya") {
        ...
        ctx.reply(`Pilih dompet (Hanya menampilkan 5 dompet yang memiliki aktivitas terbaru ${transactionType === TransactionType.OUT && "dan saldo yang cukup"})`, Markup.keyboard(filteredWallets.map((e) => e.name))
            .resize()
            .oneTime());
        return ctx.wizard.next();
    }
...
async (ctx) => {
    ...
    if (!walletId) {
        ctx.reply("Dompet tidak ditemukan, silahkan pilih dompet yang tersedia", Markup.keyboard(wallets.map((e) => e.name))
            .resize()
            .oneTime());
